		...
		if ($_GET['fpago_M'] == 100){
			...
			$pac_query = mysql_query($pac_sql);
				...
				if ($_GET['fpago_M'] == 300){
					...
					$zosema = "SELECT ZO, SE, MA, descripcion FROM ZOSEMA WHERE estado = 1 AND ZO != '777' AND ZO != '888' AND ZO != '0'";
					...
					while ($numm = mysql_fetch_array($zo_query)){
						echo '<option value="'.$numm["ZO"].'-'.$numm["SE"].'-'.$numm["MA"].'">'.$numm["ZO"].'-'.$numm["SE"].'-'.$numm["MA"].' '.$numm['descripcion'].'</option>';
					}
					echo '</select><br /><br />';
