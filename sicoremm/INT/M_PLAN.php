...
tiempo AS PERIODO_ATEN_SIN_COPAGO,tipo_plan_desc AS TIPO_PLAN,estado AS ESTADO,f_ingreso AS FECHA_INGRESO,n_inco_grat, tipo_moneda AS TIPO_MONEDA
	...
	while ($valores = mysql_fetch_array($query)) {
		...
		<form action="INT/M_PLAN.php" method="post" id="editVenCon" name="editVenCon">
			...
			<div class="caja">
				...
				<input type="text" name="cod_plan" size="3" maxlength="3" style="display:none"
					value="<?php echo $valores['COD_PLAN']; ?>" />
				<input type="text" name="tipo_plan" value="<?php echo $_GET['tipo_plan']; ?>" style="display:none" />

				<table>
					<tr>
						<td><label for="ContactName">Nombre</label><input type="text"
								value="<?php echo $valores['NOMBRE_PLAN']; ?>" name="desc_plan" /></td>
						<td><label for="ContactName">Valor Copago</label></td>
						<td><input type="text" name="copago" value="<?php echo $valores['VALOR_COPAGO']; ?>" /></td>
					</tr>
					...
					<tr>
						...
						<td><input value="<?php echo $valores['ATEN_SIN_COPAGO']; ?>" type="text" size="2" maxlength="2"
								name="cm_gratis" />&nbsp;

							<select name="tiempo">
								<option value="<?php echo strtoupper($valores['PERIODO_ATEN_SIN_COPAGO']); ?>">
									<?php echo $valores['PERIODO_ATEN_SIN_COPAGO']; ?>
								</option>
					...
					<tr>
						<td><strong>V_INCOR</strong>&nbsp;<input type="text" name="v_incor"
								value="<?php echo $valores['v_incor']; ?>" /></td>
						<td><strong>N_INCO_SP</strong></td>
						<td><input type="text" name="n_inco_grat" value="<?php echo $valores['n_inco_grat']; ?>" </td>
					</tr>
					<tr>
						<td>
							<label for="ContactName">Moneda</label>&nbsp;&nbsp;
							<? $valores['TIPO_MONEDA'] ?>
							<select name="id_tipo_moneda">
								<option value="1" <?php echo ($valores['TIPO_MONEDA'] == 'Peso') ? 'selected' : ''; ?>>
									Peso
								</option>
								<option value="2" <?php echo ($valores['TIPO_MONEDA'] == 'UF') ? 'selected' : ''; ?>>
									UF
								</option>
							</select>
						</td>
...
if ((isset($_GET['VER']))) {
	...
	$campos = array("cod_plan AS COD_PLAN" => "", "desc_plan AS NOMBRE_PLAN" => "", "copago AS VALOR_COPAGO" => "", "casa_p AS CASA_PROTEGUIDA" => "", "cm_gratis AS ATEN_SIN_COPAGO" => "", "tiempo AS PERIODO_ATEN_SIN_COPAGO" => "", "tipo_plan_desc AS TIPO_PLAN" => "", "estado AS ESTADO" => "", "f_ingreso AS FECHA_INGRESO" => "", "v_incor AS V_INORP" => "", 'n_inco_grat AS N_INCO_SN' => '','tipo_moneda'=>'');
	$rut = array('NULL' => "");
...
if (isset($_POST['ff_indreso'])) {
	...
	if (mysql_query($datos->query)) {
		...
		$rut = array("NULL" => "");
		$campos = array("cod_plan AS COD_PLAN" => "", "desc_plan AS NOMBRE_PLAN" => "", "copago AS VALOR_COPAGO" => "", "casa_p AS CASA_PROTEGUIDA" => "", "cm_gratis AS ATEN_SIN_COPAGO" => "", "tiempo AS PERIODO_ATEN_SIN_COPAGO" => "", "tipo_plan_desc AS TIPO_PLAN" => "", "estado AS ESTADO" => "", "f_ingreso AS FECHA_INGRESO" => "", "tipo_moneda" => "");
