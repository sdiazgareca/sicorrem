...
/* ELIMINAR */
	if ( isset($_GET['ELIMINAR']) ){
		...
		$query_sql = "DELETE FROM sicoremm2.vendedor WHERE nro_doc='".$_GET['RUT']."'";
...
?>
 ...
 $sql="SELECT v_cat.cod,v_cat.desc FROM v_cat ORDER BY cod DESC";
 $query = mysql_query($sql);



 ?>
 <td>CATEGORIA <select name="cat">
         <?php
         while($cat = mysql_fetch_array($query)){
         ?>
         <option value="<?php echo $cat['cod']; ?>"><?php echo $cat['desc']; ?></option>
         <?php
         }
         ?>
     </select></td>
...
<td>Domicilio de Trabajo</td><td><input type="text" name="t_domicilio" value="<?php echo $vend['t_domicilio'];?>" /></td>
 ...
 $sql="SELECT v_cat.cod,v_cat.desc FROM v_cat ORDER BY cod ASC";
 $query = mysql_query($sql);
