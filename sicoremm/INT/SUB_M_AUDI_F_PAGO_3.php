		...
		$mes = (date('m')) + 1;
			if ($mes > 12){
				...
				//$fecha_q = (date('Y')+1).'-'.$mes.'-'.date('d');
                                $fecha_q = '01-'.$mes.'-'.(date('Y')+1);
			}
	}

	echo '<br />';

	echo '<strong style="font-size:12px;"> Monto $ '.$_GET['monto'].' <br />Mes Inicial Cuota Mes <input type="text" name="ff_m_pago_i" value="'.$fecha_q.'" /></strong>
