...
$query = "UPDATE afiliados, fichas SET fichas.isapre = afiliados.obra_numero  WHERE afiliados.nro_doc = fichas.nro_doc";
$p_query = mysql_query($query);
...
//ELIMINA LOS POST NO USADOS
   ...
   foreach($_POST AS $campo => $valor){
        ...
        if ($valor != "" && $campo != 'ff_f_inicio'  &&  $campo != 'ff_f_termino' && $campo != 'ff_listado_med'  && $valor != 'TODOS' && $campo != 'importe' && $campo != 'nboleta' && $campo != 'grabar' && $campo != 'protocolo'){
   ...
   if ($condicion2 != "" AND $entre != ""){
    ...
    $query = "SELECT destino.destino,fichas.correlativo, copago.boleta,tipo_pago.tipo_pago,
              DATE_FORMAT(copago.fecha,'%d-%m-%Y') as fecha, copago.importe, fichas.paciente,copago.folio_med,
              fichas.paciente,fichas.nro_doc,fichas.num_solici,fichas.cod_plan,fichas.tipo_plan,
              titulares.nombre1 AS nombre1_t, titulares.apellido AS apellido_t, titulares.nro_doc AS nro_doc_t,
              obras_soc.reducido, empresa.empresa AS emp,fichas.observacion
    ...
    if ($nums > 0){
            ...
            <table class="table4">
                ...
                <td><a href="INT/M_TESO_MED.php?Editar=1&protocolo=<?php echo $copagos['correlativo']; ?>" class="boton">Editar</a></td>
                    ...
                    <div style="background:#C6E8EC; width:900px;padding:5px 0px 5px 0px; font-size:9px;"><strong>ESTADO</strong> <?php echo $copagos['destino']; ?></div>
