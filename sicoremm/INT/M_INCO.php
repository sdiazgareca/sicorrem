...
function formato_monto($monto, $tipo_moneda)
{
    if (!is_numeric($monto)) {
        return '---';
    }
    ...
    if ($tipo_moneda == 1) {
        ...
        // UF con 2 decimales
        return number_format($monto, 2, ',', '.') . ' UF';
    } else {
        return $monto;
    }
}
...
if ($_POST['num_solici'] > 1) {
    ...
    if ($num > 0) {
        ...
        while ($cont = mysql_fetch_array($query)) {
            ...
            echo "<td><strong>V_INCORPORACION</strong></td><td>" . formato_monto($cont['v_incor'], 1) . "</td>";


            // Formatea UF o PESO
            if (isset($cont['id_tipo_moneda']) && isset($cont['mensualidad'])) {
                $monto = formato_monto($cont['mensualidad'], $cont['id_tipo_moneda']);
            } else {
                $monto = '---';
            }


            echo "<td><strong>MONTO</strong></td><td>" . $monto . "</td>";
            echo "<td><strong>AJUSTE</strong></td><td>" . formato_monto($cont['ajuste'], $cont['id_tipo_moneda']) . "</td>";
