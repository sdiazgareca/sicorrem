...
$fecha = new Datos();
$fecha_rendicion = $fecha->cambiaf_a_mysql($_POST['periodo']);
...
WHERE t_mov.codigo !=98 AND t_mov.codigo !=99 AND t_mov.codigo !=100 AND (cobrador.codigo='".$_POST['cobrador']."' || cta.cobrador='".$_POST['cobrador']."') AND rendicion='".$_POST['rendicion']."'
...
/* ZONA DE COBRO COBRADOR */
    ...
    if (($zona['empresa'] == "" || $zona['empresa'] < 1) && $zona['tipo_plan'] != "3" && $zona['fecha_mov'] == $fecha_rendicion && $zona['cobrador_asignado'] == $zona['cobrador_rend'] && $zona['afectacion'] > 0){
        $periodo_Actual_comprovante = $periodo_Actual_comprovante.' <div style="font-size: 12px; font-weight: bold; ">A1'.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
        $periodo_actual_total = $periodo_actual_total + $zona['importe'];
        $periodo_Actual_cuenta ++;
    }
    //PERIODO ANTERIOR COBRADO ZONA PAGADO
    if (($zona['empresa'] == "" || $zona['empresa'] < 1) && ($zona['tipo_plan'] != "3") && ($zona['fecha_mov'] < $fecha_rendicion) && ($zona['cobrador_asignado'] == $zona['cobrador_rend']) && ($zona['afectacion'] > 0) ){
        $periodo_Anterior_comprovante = $periodo_Anterior_comprovante.' <div style="font-size: 12px; font-weight: bold; ">A2'.$periodo_Anterior_comprovante.' '.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
    ...
    if (($zona['empresa'] == "" || $zona['empresa'] < 1) && $zona['tipo_plan'] != "3" && $zona['fecha_mov']  > $fecha_rendicion && $zona['cobrador_asignado'] == $zona['cobrador_rend'] && $zona['afectacion'] > 0){
        $periodo_Anticipado_comprovante = $periodo_Anticipado_comprovante.' <div style="font-size: 12px; font-weight: bold; ">A3'.$periodo_Anticipado_comprovante.' '.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
...
/* TC O MANDATO */
    ...
    //PERIODO ANTERIOR COBRADO ZONA PAGADO
    if (($zona['empresa'] == "" || $zona['empresa'] < 1) && $zona['tipo_plan'] != "3" && $zona['fecha_mov']  < $fecha_rendicion && $zona['cobrador_asignado'] != $zona['cobrador_rend'] && $zona['afectacion'] > 0){
        $periodo_Anterior_comprovante_OT = $periodo_Anterior_comprovante_OT.'<div style="font-size: 12px; font-weight: bold; ">'.$periodo_Anterior_comprovante.' '.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
    ...
    //PERIODO ANTERIOR COBRADO ZONA PAGADO
    if (($zona['empresa'] == "" || $zona['empresa'] < 1) && $zona['tipo_plan'] == "3" && $zona['fecha_mov']  < $fecha_rendicion && $zona['cobrador_asignado'] == $zona['cobrador_rend'] && $zona['afectacion'] > 0){
        $periodo_Anterior_comprovante_AP = $periodo_Anterior_comprovante_AP.'<div style="font-size: 12px; font-weight: bold; ">'.$periodo_Anterior_comprovante.' '.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
    ...
    //PERIODO ANTICIPADO COBRADO ZONA PAGADO
    if (($zona['empresa'] == "" || $zona['empresa'] < 1) && $zona['tipo_plan'] == "3" && $zona['fecha_mov']  > $fecha_rendicion && $zona['cobrador_asignado'] == $zona['cobrador_rend'] && $zona['afectacion'] > 0){
        $periodo_Anticipado_comprovante_AP = $periodo_Anticipado_comprovante_AP.'<div style="font-size: 12px; font-weight: bold; ">'.$periodo_Anticipado_comprovante.' '.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
    ...
    //PERIODO ANTERIOR COBRADO ZONA PAGADO
    if (($zona['empresa'] != "" || $zona['empresa'] > 0) && $zona['tipo_plan'] != "3" && $zona['fecha_mov']  < $fecha_rendicion && $zona['cobrador_asignado'] == $zona['cobrador_rend'] && $zona['afectacion'] > 0){
        $periodo_Anterior_comprovante_CONV = $periodo_Anterior_comprovante_CONV.'<div style="font-size: 12px; font-weight: bold; ">'.$periodo_Anterior_comprovante.' '.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
    ...
    //PERIODO ANTICIPADO COBRADO ZONA PAGADO
    if (($zona['empresa'] != "" || $zona['empresa'] > 0) && $zona['tipo_plan'] != "3" && $zona['fecha_mov']  > $fecha_rendicion && $zona['cobrador_asignado'] == $zona['cobrador_rend'] && $zona['afectacion'] > 0){
        $periodo_Anticipado_comprovante_CONV = $periodo_Anticipado_comprovante_CONV.'<div style="font-size: 12px; font-weight: bold; ">'.$periodo_Anticipado_comprovante.' '.$zona['comprovante'].' -  $ '.$zona['importe'].' CONTRATO '.$zona['num_solici'].'</div>';
