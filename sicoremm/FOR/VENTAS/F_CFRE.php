...
if ( isset($_POST['codigo'])){
	$f_pago_sql ="UPDATE sicoremm2.f_pago SET comi_free='".$_POST['valor']."' WHERE `codigo`='".$_POST['codigo']."'";
...
echo '<h1>Tabla de Comisiones Vendedores Free Lance</h1>';
...
while ($f_pago = mysql_fetch_array($f_pago_query)){
		  ...
		  ?>
		  <form action="FOR/VENTAS/F_CFRE.php" method="post" name="<? echo 'na_'.$i; ?>" id="<? echo 'id_'.$i; ?>">
		  ...
		  <td style="width:50px"><input type="text" size="5" maxlength="3" value="<?php echo $f_pago['comi_free']; ?>" name="valor"></td>
          <td>%</td>
          <td><input type="submit" value="Guardar" class="boton"></td>
          <!--  
          <td><strong>Locomoci&oacute;n</strong></td>
