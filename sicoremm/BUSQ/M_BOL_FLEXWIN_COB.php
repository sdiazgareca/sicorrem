...
$s = "UPDATE cta SET fecha_vto = CONCAT(YEAR(fecha_vto),'-',MONTH(fecha_vto),'-','10')";
$q=mysql_query($s);


$sql="SELECT tipo_comp, DATE_FORMAT(fecha_vto,'%d-%m-%Y') as fecha_vto, comprovante, haber, nro_doc,fecha 
      FROM cta
      INNER JOIN t_mov ON t_mov.codigo = cta.cod_mov
      WHERE cta.fecha BETWEEN '".$periodo1."' AND '".$periodo2."'  AND t_mov.operador = 'H'";
