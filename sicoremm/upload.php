...
$tamano_archivo = $_FILES['userfile']['size'];
	...
	if ( ($tipo_archivo != "application/vnd.ms-excel") && ($tipo_archivo != "text/csv")) {
            ...
            if (move_uploaded_file($_FILES['userfile']['tmp_name'], $nombre_archivo)){
				...
				$tabla= '<table class="table2"><tr><th>ESTADO</th><th>CONTRATO</th><th>RUT</th><th>PLAN</th><th>TIPO PLAN</th></tr>';
					...
					switch($datos[7]){case 'TITULAR':$parentesco='100';break;case 'ESPOSA':$parentesco='200';break;case 'CONVIVIENTE':$parentesco='600';break;case 'HIJASTRO':$parentesco='700';break;case 'HIJO(A)':$parentesco='300';break;case 'HIJO_INV':$parentesco='400';break;case 'MADRE':$parentesco='800';break;case 'MED.PROTEC':$parentesco='500';break;}
                                        ...
                                        $sql_isapre = "SELECT nro_doc FROM obras_soc WHERE nro_doc = ".$isapre;
                                        $query_sql = mysql_query($sql_isapre);
                                        $salIsapre = mysql_fetch_array($query_sql);

                                        if(is_numeric($salIsapre['nro_doc']) == FALSE ){
                                            echo '<strong>ERROR ESTRUCTURA DE ARCHIVOS</strong>';
                                            exit;
                                        }
                                        	...
                                        	if ($datos[7] =='TITULAR'){
                                                ...
                                                $com_num_solici_sql ="SELECT COUNT(contratos.titular) AS num, num_solici
                                                    FROM contratos
                                                    WHERE contratos.titular='".$datos[0]."' AND num_solici >= 70000
                                                        GROUP BY contratos.titular";
                                                ...
                                                $com_num_solici = mysql_fetch_array($com_num_solici_query);
                                                    ...
                                                    if($com_num_solici['num'] > 0){
                                                        ...
                                                        $cambia_isapre = 'UPDATE afiliados SET afiliados.cod_baja="00", obra_numero="'.$isapre.'", obra_afi="'.$isapre.'", fecha_nac="'.$f_nac.'" WHERE nro_doc="'.$datos[0].'"';
                                                        $cambia_contrato ='UPDATE contratos SET estado = 500 where num_solici="'.$com_num_solici['num_solici'].'"';

 
                                                        if (mysql_query($cambia_isapre)){
                                                            mysql_query($cambia_contrato);
                                                    ...
                                                    if($com_num_solici['num'] < 1){
                                                        ...
                                                        $agregarTITULAR = "INSERT INTO afiliados (apellido,nombre1,nombre2,sexo,fecha_alta, fecha_act,fecha_ing,fecha_nac,nro_doc,pais,cod_parentesco,cod_baja, categoria,num_solici,titular,cod_plan,tipo_plan,obra_numero,obra_afi) VALUES('".$datos[2]."','".$datos[3]."','".$datos[4]."','".$datos[5]."','".date("Y-m-d")."', '".date("Y-m-d")."','".date("Y-m-d")."','".$f_nac."','".$datos[0]."','600','100','00','1','".$conMED."','".$datos[9]."', 'W71','2','".$isapre."','".$isapre."')";
        ...
        if ($datos[7] !='TITULAR'){
                                   ...
                                   $agregarTITULAR = "INSERT INTO afiliados (apellido,nombre1,nombre2,sexo,fecha_alta,fecha_act,fecha_ing,fecha_nac,nro_doc,pais,cod_parentesco,cod_baja,categoria,num_solici,titular,cod_plan,tipo_plan,obra_numero,obra_afi) VALUES('".$datos[2]."','".$datos[3]."','".$datos[4]."','".$datos[5]."','".date("Y-m-d")."','".date("Y-m-d")."','".date("Y-m-d")."','".$f_nac."','".$datos[0]."','600','".$parentesco."','00','2','".$com_num_solici2['num_solici']."','".$datos[9]."','W71','2','".$isapre."','".$isapre."')";
                                   ...
                                   if (mysql_query($agregarTITULAR)){
                                            ...
                                            $query9 = mysql_query($cambia_contrato);
                                            }
