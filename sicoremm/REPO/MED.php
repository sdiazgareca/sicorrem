...
?>
<?php
echo '<strong>MEDIMEL ENTREGA SEMANA '.$fechainicio[0].'-'.$fechainicio[1].'-'.$fechainicio[2].' al '.$fechatermino[0].'-'.$fechatermino[1].'-'.$fechatermino[2].'</strong>';


$listPlan_sql ="
SELECT fichas.correlativo, copago.boleta,tipo_pago.tipo_pago,DATE_FORMAT(copago.fecha,'%d-%m-%Y') AS fecha, copago.importe, fichas.paciente,copago.folio_med, fichas.paciente,fichas.nro_doc,fichas.num_solici,
titulares.nombre1 AS nom_t, titulares.apellido AS ape_t, contratos.titular,
obras_soc.descripcion AS isapre2
FROM copago
...
LEFT JOIN obras_soc ON obras_soc.nro_doc = fichas.isapre
WHERE contratos.empresa='15004439' AND copago.fecha BETWEEN '".$_ini."' AND '".$_ter."' ORDER BY copago.folio_med,copago.boleta";
...
<td><strong>RUT</strong></td>
<td>&nbsp;</td>
<td><strong>SISTEMA SALUD</strong></td>
...
<td><?php echo $bus['isapre2']; ?></td>
<td><?php echo $bus['folio_med']; ?></td>
</tr>
<?php
}
?>
