...
if($_POST['editar_direccion'] > 0){
    ...
    foreach($_POST AS $CAMPO=>$VALOR){
        ...
        if($CAMPO != 'nro_doc' && $CAMPO != 'num_solici' && $CAMPO != 'tipo_dom' && $CAMPO != 'editar_direccion'){
...
$query = $query.' WHERE nro_doc ="'.$_POST['nro_doc'].'" AND num_solici="'.$_POST['num_solici'].'" AND tipo_dom = "'.$_POST['tipo_dom'].'"';
...
$_POST['fecha_nac'] = $fecha;
    ...
    foreach ($_POST AS $campo=>$valor){
        ...
        if ($campo != 'cambiar_titu' && $campo != 'rut_original' && $campo != 'num_solici'){
    ...
    $sql ="SELECT nro_doc from titulares where nro_doc = '".$_POST['nro_doc']."'";
    $query = mysql_query($sql);

    $num = mysql_num_rows($query);

    if ($num > 0){
        $query2['titulares'] = 'UPDATE titulares SET'.$camm.' WHERE nro_doc="'.$_POST['nro_doc'].'"';
