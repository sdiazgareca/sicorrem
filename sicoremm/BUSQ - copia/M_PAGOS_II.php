...
while ($cta = mysql_fetch_array($cta_query)){
    ...
    echo '<tr>
            ...
            <td><input type="checkbox" name="'.$cta['comprovante'].$cta['num_solici'].'"  value="'.$cta['titular'].'/'.$cta['factu'].'/'.$cta['comprovante'].'/'.$t_mov.'/'.$cta['fecha_mov'].'/'.$cta['valor'].'/'.$cta['num_solici'].'/'.$_POST['rendicion'].'/'.$_POST['fecha'].'/'.$_POST['cobrador'].'" ></td>
