...
AND contratos.factu='C'
AND (contratos.estado ='500' || contratos.estado ='400' || contratos.estado =  '3500')
AND (f_pago != '600' || f_pago !=400) AND contratos.tipo_plan != 5 AND contratos.tipo_plan != 2
...
//COMPROBAR REAJUSTE
    ...
    if ($fac['f_pago'] == '100' || $fac['f_pago'] == '200'){
        ...
        if ($num_deudas >= 6){
            ...
            echo '<tr><td>'.$i.'</td><td>'.$fac['num_solici'].'</td><td>'.$fac['titular'].'</td><td>'.$fac['nom'].' '.$fac['ape'].'</td><td>'.$fac['f_ingreso'].'</td><td>'.$ajuste.'</td><td>'.$fac['ZO'].'</td><td>'.$fac['SE'].'</td><td>'.$fac['MA'].'</td><td>'.strtoupper($fac['descripcion']).'</td><td>'.$fac['calle'].'</td><td>'.$fac['numero'].'</td><td>'.$fac['poblacion'].'</td><td>'.$fac['casa'].'</td><td>'.$fac['departamento'].'</td><td>'.$fac['telefono'].'</td><td>'.$fac['pasaje'].'</td><td>'.$fac['f_pago_des'].'</td><td>'.$fac['ap_cob'].'</td><td>'.$fac['nom_cob'].'</td><td>'.$num_deudas.'</td><td><strong>DICOM</strong></td><td>'.$fac['importe'].'</td><td>'.$re_ajuste.'</td><td>'.$total.'</td><td>'.$fac['secuencia'].'</td></tr>';
            //PASAR A CLIENTE MOROSO
            $i ++;
        }
        else{
            echo '<tr><td>'.$i.'</td><td>'.$fac['num_solici'].'</td><td>'.$fac['titular'].'</td><td>'.$fac['nom'].' '.$fac['ape'].'</td><td>'.$fac['f_ingreso'].'</td><td>'.$ajuste.'</td><td>'.$fac['ZO'].'</td><td>'.$fac['SE'].'</td><td>'.$fac['MA'].'</td><td>'.strtoupper($fac['descripcion']).'</td><td>'.$fac['calle'].'</td><td>'.$fac['numero'].'</td><td>'.$fac['poblacion'].'</td><td>'.$fac['casa'].'</td><td>'.$fac['departamento'].'</td><td>'.$fac['telefono'].'</td><td>'.$fac['pasaje'].'</td><td>'.$fac['f_pago_des'].'</td><td>'.$fac['ap_cob'].'</td><td>'.$fac['nom_cob'].'</td><td>'.$num_deudas.'</td><td><strong>FACTURAR</strong></td><td>'.$fac['importe'].'</td><td>'.$re_ajuste.'</td><td>'.$total.'</td><td>'.$fac['secuencia'].'</td></tr>';
    ...
    if ($fac['f_pago'] == '300' || $fac['f_pago'] == '500'){
        ...
        if ($num_deudas >= 3){
            ...
            /*
            $sql ="SELECT comprovante FROM cta WHERE YEAR(cta.fecha_mov)='' AND MONTH(cta.fecha_mov)='' AND cta.nro_doc='' AND num_solici=''";
            $query = mysql_query($sql);
